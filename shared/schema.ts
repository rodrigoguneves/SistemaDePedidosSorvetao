...
// Order Items model (Updated for new schema)
...
// Define all the relations after all tables are defined to avoid circular references
...
export const customerRelations = relations(customers, ({ many }) => ({
  ...
  categoryAllowedSaleUnits: many(customerCategoryAllowedSaleUnits),
}));

export const productCategoryRelations = relations(productCategories, ({ many }) => ({
  baseProducts: many(baseProducts),
  customerCategories: many(customerCategories),
  products: many(products), // For backwards compatibility
}));

export const saleUnitRelations = relations(saleUnits, ({ many }) => ({
  productSaleVersions: many(productSaleVersions),
  customerCategoryAllowedSaleUnits: many(customerCategoryAllowedSaleUnits),
}));

export const baseProductRelations = relations(baseProducts, ({ one, many }) => ({
  category: one(productCategories, {
    fields: [baseProducts.product_category_id],
    references: [productCategories.id],
  }),
  saleVersions: many(productSaleVersions),
}));

export const productSaleVersionRelations = relations(productSaleVersions, ({ one, many }) => ({
  baseProduct: one(baseProducts, {
    fields: [productSaleVersions.base_product_id],
    references: [baseProducts.base_product_id],
  }),
  saleUnit: one(saleUnits, {
    fields: [productSaleVersions.sale_unit_id],
...
export const orderItemRelations = relations(orderItems, ({ one }) => ({
  ...
  productVersion: one(productSaleVersions, {
    fields: [orderItems.product_version_id],
    references: [productSaleVersions.product_version_id],
  }),
}));
...
export type SaleUnit = typeof saleUnits.$inferSelect;
export type InsertSaleUnit = z.infer<typeof insertSaleUnitSchema>;

export type BaseProduct = typeof baseProducts.$inferSelect;
export type InsertBaseProduct = z.infer<typeof insertBaseProductSchema>;

export type ProductSaleVersion = typeof productSaleVersions.$inferSelect;
export type InsertProductSaleVersion = z.infer<typeof insertProductSaleVersionSchema>;
...
export type CustomerCategoryAllowedSaleUnit = typeof customerCategoryAllowedSaleUnits.$inferSelect;